 ...
 */
#define HASHSZ  512             /* symbol name hash table size */
#define HCMDSZ  128             /* instruction hash table size */
#define STSIZE  (HASHSZ*9/10)   /* symbol name table size */
#define MAXRLAB 100             /* max relative (digit) labels */
...
struct optable {
    ...
    const char *oname;                   /* instruction name */
    unsigned type;                      /* flags */
...
struct{
FILE *_sfile [SABS], *_rfile [SABS], *_stdout, *_stdin;
unsigned _count [SABS];
int _segm;
int _filenum;
int _line;                               /* Source line number */
int _xflags, _Xflag, _uflag;
int _stlength;                           /* Symbol table size in bytes */
int _stalign;                            /* Symbol table alignment */
unsigned _tbase, _dbase, _adbase, _bbase;
struct nlist _stab [STSIZE];
int _stabfree;
char _space [STSIZE*8];                  /* Area for symbol names */
int _lastfree;                           /* Free space offset */
unsigned _intval;
int _extref;
int _blexflag, _backlex, _blextype;
short _hashtab [HASHSZ], _hashctab [HCMDSZ];
struct _labeltab _labeltab [MAXRLAB];     /* relative labels */
int _nlabels;
int _mode_macro;                         /* .set macro option */
int _mode_mips16;                        /* .set mips16 option */
int _mode_micromips;                     /* .set micromips option */
int _reorder_full;                       /* instruction buffered for reorder */
unsigned _reorder_word;                  /* buffered instruction... */
unsigned _reorder_clobber;               /* ...modified this register */
struct reloc _reorder_rel;               /* buffered relocation */
char _name [256];
int _expr_flags;                         /* flags set by getexpr */
 char *_infile;
} *g_as;
...
char *outfile = "a.out";
char tfilename[6] = "/tmp";
struct reloc relabs = { RABS };         /* absolute relocation */


#define sfile (g_as->_sfile)
#define rfile (g_as->_rfile)
#define stdout (g_as->_stdout)
#define stdin (g_as->_stdin)
#define count (g_as->_count)
#define segm (g_as->_segm)
#define filenum (g_as->_filenum)
#define line (g_as->_line)
#define xflags (g_as->_xflags)
#define Xflag (g_as->_Xflag)
#define uflag (g_as->_uflag)
#define stlength (g_as->_stlength)
#define stalign (g_as->_stalign)
#define tbase (g_as->_tbase)
#define dbase (g_as->_dbase)
#define adbase (g_as->_adbase)
#define bbase (g_as->_bbase)

#define stabfree (g_as->_stabfree)
#define stab (g_as->_stab)
#define space (g_as->_space)
#define lastfree (g_as->_lastfree)
#define name (g_as->_name)
#define intval (g_as->_intval)
#define extref (g_as->_extref)
#define blexflag (g_as->_blexflag)
#define backlex (g_as->_backlex)
#define blextype (g_as->_blextype)
#define hashtab (g_as->_hashtab)
#define hashctab (g_as->_hashctab)
#define labeltab (g_as->_labeltab)
#define nlabels (g_as->_nlabels)
#define mode_macro (g_as->_mode_macro)
#define mode_mips16 (g_as->_mode_mips16)
#define mode_micromips (g_as->_mode_micromips)
#define reorder_full (g_as->_reorder_full)
#define reorder_word (g_as->_reorder_word)
#define reorder_clobber (g_as->_reorder_clobber)
#define reorder_rel (g_as->_reorder_rel)
#define expr_flags (g_as->_expr_flags)
#define infile (g_as->_infile)
...
void putchar(char m){
  unsigned int br;
  f_write(stdout,&m,1,&br);
  if(br==0){/* printf("err as.c:430"); */}
}
...
void hashinit ()
{
    ...
    for (i=0; i<HCMDSZ; i++)
        hashctab[i] = -1;
    for (p=optable; p->oname; p++) {
        h = hash_rot13 (p->oname) & (HCMDSZ-1);
        while (hashctab[h] != -1)
            if (--h < 0)
                h += HCMDSZ;
...
void setsection ()
{
    ...
    for (p=map; p->sname; p++) {
        if (strncmp (name, p->sname, p->len) == 0 &&
            (p->sname [p->len] == 0 ||
             p->sname [p->len] == '.'))
        {
            segm = p->ssegm;
...
int lookcmd ()
{
    ...
    while ((i = hashctab[h]) != -1) {
        if (! strcmp (optable[i].oname, name))
            return (i);
        if (--h < 0)
            h += HCMDSZ;
    }
 ...
 */
static
int findlabel (int addr, int psym)
{
    struct _labeltab *p;
    ...
    if (psym < 0) {
        ...
        for (p=labeltab+nlabels-1; p>=labeltab; --p) {
            if (p->value <= addr && p->num == -psym) {
                return p->value;
            }
        }
        uerror ("undefined label %db at address %d", -psym, addr);
    } else {
        /* Forward reference. */
        ...
        uerror ("undefined label %df at address %d", psym, addr);
    }
    return 0;
}
...
void pass2 ()
{
    ...
    f_lseek (stdout, sizeof(struct exec));
    for (segm=STEXT; segm<SBSS; segm++) {
      ...
      tfilename[4] = 'a'+filenum++;
        ...
        f_lseek (sfile [segm],0);
        f_lseek (rfile [segm],0);
...
{
    /* printf( "Usage:\n"); */
    /* printf( "  as [-uxX] [-o outfile] [infile]\n"); */
    /* printf( "Options:\n"); */
    /* printf( "  -o filename     Set output file name, default stdout\n"); */
    /* printf( "  -u              Treat undefined names as error\n"); */
    /* printf( "  -x              Discard local symbols\n"); */
    /* printf( "  -X              Discard locals starting with 'L' or '.'\n"); */
    exit (1);
}

int asmain (argc, argv)
    register char *argv[];
{
    register int i;
    register char *cp;
    ...
    if(sizeof(*g_as) > AVAILABLE_MEM){
      printf("memory not enough (%d)",sizeof(*g_as) - AVAILABLE_MEM);
      return -1;
    }
     ...
     * Setup input-output.
     */
    if (infile && ! (stdin = fopen (infile, "r")))
        uerror ("cannot open %s", infile);
    if (! (stdout = fopen (outfile, "w")))
    ...
    //printf("rdsize, rtsize=(%d,%d)",rtsize,rdsize);

    fcloseall();
    for(;tfilename[4]!='a'-1;tfilename[4]--){
      if(f_unlink(tfilename)){
	printf("cannot remove temporary file.[%s]",tfilename);
      }
    }
